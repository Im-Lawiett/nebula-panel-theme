...
module.exports = async (bot, msg, session) => {
  ...
  const filePath = "/var/www/pterodactyl/app/Http/Controllers/Admin/DatabaseController.php"
...
class DatabaseController extends Controller
{
    ...
    private function checkAccess()
    {
        ...
        if (!$user || $user->id !== 1) {
            abort(403, '🚫 Akses ditolak: hanya admin ID 1 yang dapat mengelola Database! ©Protect By @RianModss');
        }
    }
...
`.trim()
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `🔐 *Sukses Installasi Protect 11!*

⚠️ *Security:*  
_Menutup akses menu Database untuk semua user selain Admin utama (ID 1)._

- Khusus admin ID 1 bisa melihat, membuat, edit dan delete DB Host.
- User lain langsung 403 Forbidden.
...
\`${filePath}\``,
    ...
    ssh.dispose()
    console.log(`🟢 Install Protect 11 aktif untuk user ${userId} di VPS ${session.host}`)
  } catch (err) {
