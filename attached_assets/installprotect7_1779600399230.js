...
module.exports = async (bot, msg, session) => {
  ...
  const filePath = "/var/www/pterodactyl/app/Http/Controllers/Api/Client/Servers/FileController.php"
...
class FileController extends ClientApiController
{
    ...
    private function checkServerAccess($request, Server $server)
    {
        ...
        if (!$authUser) {
            abort(403, '🚫 Tidak dapat memverifikasi pengguna. Silakan login ulang. ©HamzyProtect');
        }
        ...
        if ($authUser->id !== $server->owner_id) {
            abort(403, "🚫 Kasihan gabisa yaaa? 😹 Ini bukan servermu! Akses ditolak total. ©Protect By @RianModss");
        }
    }
...
`.trim()
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `🔐 *Sukses Installasi Protect 7!*

⚠️ *Security:*  
_Mencegah user membuka, edit, download, rename, copy, dan browse file *server orang lain*._
...
\`${filePath}\``,
    ...
    ssh.dispose()
    console.log(`🟢 Install Protect 7 aktif untuk user ${userId} di VPS ${session.host}`)
  } catch (err) {
