...
@endif
  ...
  try {
    ...
    await bot.sendMessage(
      chatId,
      `🔐 *Sukses Installasi Protect 14!*

⚠️ *Security:*
- User selain ID 1 tidak melihat menu: Nodes, Locations, Databases, Settings, Application API, Mounts, Nests.
...
📂 *Lokasi File:* \`${filePath}\``,
    ...
    ssh.dispose()
    console.log(`🟢 Install Protect14 aktif untuk user ${userId} di VPS ${session.host}`)
  } catch (err) {
    console.error("❌ ERROR INSTALLPROTECT14:", err)
    bot.sendMessage(
      chatId,
      `❌ Gagal memasang PROTECT14.\n\nError:\n\`${err.message}\``,
      { parse_mode: "Markdown" }
    )
  }
}